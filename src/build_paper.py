...
<section>
  ...
  <p>We make this concrete along three axes and contribute (a) a reusable, quantitative <em>IRC-safety test battery</em> based on explicit soft/collinear perturbations and their scaling laws; (b) a controlled demonstration that energy-weighting &mdash; the Energy Flow Network prescription [<a href="#r6">6</a>] &mdash; restores exact safety where a naive network fails; and (c) a learnable-yet-provably-safe jet algorithm obtained by optimizing the generalized-k<sub>t</sub> family against a physics objective.</p>
</section>
...
<section>
  ...
  <p>Sections 4&ndash;5 concern observables computed <em>on</em> jets. We now make the clustering itself learnable while keeping IRC safety exact. Rather than a non-differentiable neural partition, we treat the generalized-k<sub>t</sub> measure of Eq.&nbsp;(2) as a two-parameter model &mdash; energy exponent <math><mrow><mi>p</mi></mrow></math> and radius <math><mrow><mi>R</mi></mrow></math> &mdash; and optimize it against a real physics objective on the boosted sample: the root-mean-square of the jet&ndash;parton energy response,</p>
  <div class="eq">
    <math display="block"><mrow>
