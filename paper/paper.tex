...
soft emission scales linearly with the emission momentum and vanishes in the limit, whereas
an unconstrained Particle Flow Network of equal accuracy saturates at a finite plateau.
(iii) We build a \emph{learnable} jet algorithm by treating the generalized-$\kt$ energy
exponent and radius as free parameters and optimizing them against a genuine physics
objective (jet--parton energy response); every member of the family is IRC-safe, and the
optimum rediscovers the \antikt{} exponent $p=-1$ at a mildly enlarged radius. The unifying
...
We make this concrete along three axes and contribute (a) a reusable, quantitative
IRC-safety test battery based on the explicit perturbations~\eqref{eq:coll}--\eqref{eq:ir}
and their scaling laws; (b) a controlled demonstration that energy weighting --- the Energy
Flow Network prescription~\cite{efn} --- restores exact safety where a naive network fails;
and (c) a learnable-yet-provably-safe jet algorithm obtained by optimizing the
...
\subsection{Equal accuracy, unequal safety}
...
learnable while keeping IRC safety exact. Rather than a non-differentiable neural partition,
we treat the generalized-$\kt$ measure~\eqref{eq:genkt} as a two-parameter model --- energy
exponent $p$ and radius $R$ --- and optimize it against a real physics objective on the
boosted sample. Matching reconstructed jets to the outgoing hard partons (\textsc{Pythia}
